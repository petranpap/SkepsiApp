                    ...
                    <div class="overflow-x-auto relative shadow-md sm:rounded-lg">
                        ...
                        <table class="w-full text-sm text-center text-gray-500 dark:text-gray-400 " id='table_users'>
                            ...
                            <tr class="bg-white border-b hover:bg-teal-100 hover:text-black dark:border-white-700 text-sm text-black">
                                <td class="py-4 px-6"> {{ $intern->name }} </td>
                                <td class="py-4 px-6"> {{ $intern->email }}</td>
                                <td class="py-4 px-6"> {{ $intern->created_at }}</td>

                                <td>
                                    <a class="btn btn-primary" href="/interns/edit/{{ $intern->id}}"><img src="/icons/edit_user.png" width="50" style=" display: block; margin-left: auto; margin-right: auto; "></a>

                                </td>
                                <td>
                                    <form action="/interns/delete/{{ $intern->id}}" method="Post" id="delete_form">
                                        @csrf
                                        @method('POST')
...
<script>
    $("li:not(#all_interns_menu)").removeClass("bg-gray-100 border-double border-4 border-green-500 text-gray-700 ");
    $("#all_interns_menu").addClass("bg-gray-100 border-double border-4 border-green-500 text-blue-500 ");
    ...
    $('#delete_user_btn').click(function () {
        ...
        Swal.fire({
            title: 'Do you want to delete the Intern?',
            showDenyButton: true,
            ...
            if (result.isConfirmed) {
                Swal.fire('Intern Deleted!', '', 'error')
                setTimeout(() => {
                    $('#delete_form').submit()
                }, 1500);
            } else if (result.isDenied) {
                Swal.fire('Intern is not deleted', '', 'info')
            }
        })
    })
</script>
